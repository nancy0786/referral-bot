...
async def load_user_data(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    try:
        ...
        updates = await context.bot.get_updates(limit=100)  # Await the coroutine
        for update in updates:  # Synchronous iteration
            if (
                update.message
...
async def get_total_users(context: ContextTypes.DEFAULT_TYPE):
    ...
    count = 0
    try:
        updates = await context.bot.get_updates(limit=1000)  # Await the coroutine
        for update in updates:  # Synchronous iteration
            if (
                update.message
                and update.message.chat_id == STORAGE_CHAT_ID
