...
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, MessageId
...
async def resolve_chat_id(context: ContextTypes.DEFAULT_TYPE, chat_identifier):
    """Convert a channel link, invite link, or ID to a chat ID."""
    try:
        ...
        if chat_identifier.startswith(("@", "t.me/", "https://t.me/")):
            username = re.sub(r"^(https?://)?t\.me/|@|^/+", "", chat_identifier)
            if not username:
                raise ValueError(f"Invalid chat identifier: {chat_identifier}")
            chat = await context.bot.get_chat(f"@{username}")
            return chat.id
        if chat_identifier.startswith(("+", "t.me/+", "https://t.me/+")):
            invite_code = re.sub(r"^(https?://)?t\.me/|\+", "", chat_identifier)
            if not invite_code:
                raise ValueError(f"Invalid chat identifier: {chat_identifier}")
            chat = await context.bot.join_chat(f"https://t.me/+{invite_code}")
            return chat.id
        raise ValueError(f"Invalid chat identifier: {chat_identifier}")
    except TelegramError as e:
        logger.error(f"Error resolving chat ID for {chat_identifier}: {e}")
        raise ValueError(f"Failed to resolve chat ID for {chat_identifier}. Ensure the channel exists and the bot has access.")

# Initialize chat IDs
async def initialize_chat_ids(context: ContextTypes.DEFAULT_TYPE):
    """Resolve CHANNEL_ID and STORAGE_CHAT_ID from .env if not already resolved."""
    ...
    try:
        ...
        logger.error(f"Failed to initialize chat IDs: {e}")
        raise
...
async def load_user_data(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    try:
        if user_id in USER_MESSAGE_IDS:
            ...
            # Copy the message to a temporary chat to retrieve its content
            temp_message = await context.bot.copy_message(
                chat_id=user_id,
                from_chat_id=STORAGE_CHAT_ID,
                message_id=message_id
            )
            # Fetch the copied message to get its text
            copied_message = await context.bot.get_updates(
                offset=-1,
                limit=1,
                allowed_updates=["message"]
            )
            for update in copied_message:
                if update.message and update.message.message_id == temp_message.message_id:
                    if update.message.text and update.message.text.startswith(f"USER_{user_id}\n"):
                        ...
                        return data
        # Fallback: Search storage chat for user data
        temp_message = await context.bot.send_message(
            chat_id=STORAGE_CHAT_ID,
            text=f"SEARCH_{user_id}"
        )
        ...
        for update in updates:
            if update.message and update.message.chat_id == STORAGE_CHAT_ID:
                if update.message.text and update.message.text.startswith(f"USER_{user_id}\n"):
                    data = json.loads(update.message.text.split("\n", 1)[1])
                    USER_MESSAGE_IDS[user_id] = update.message.message_id
                    data["message_id"] = update.message.message_id
                    # Clean up temporary message
                    await context.bot.delete_message(chat_id=STORAGE_CHAT_ID, message_id=temp_message.message_id)
                    return data
        ...
        logger.error(f"Error parsing user data JSON for {user_id}: {e}")
        return None
...
async def get_total_users(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for update in updates:
            if update.message and update.message.chat_id == STORAGE_CHAT_ID:
                if update.message.text and update.message.text.startswith("USER_"):
                    count += 1
        await context.bot.delete_message(chat_id=STORAGE_CHAT_ID, message_id=temp_message.message_id)
    except TelegramError as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Initialize chat IDs if not already resolved (still strings)
    if isinstance(CHANNEL_ID, str) or isinstance(STORAGE_CHAT_ID, str):
        try:
            await initialize_chat_ids(context)
        except ValueError as e:
            await update.message.reply_text(f"Bot setup error: {str(e)}")
            return
