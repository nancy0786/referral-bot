...
async def resolve_chat_id(context: ContextTypes.DEFAULT_TYPE, chat_identifier):
    ...
    try:
        # If already a chat ID (integer), return it
        ...
        # Handle numeric chat ID (string form)
        if chat_identifier.isdigit() or chat_identifier.startswith("-"):
            return int(chat_identifier)
        
        # Handle username (e.g., @myprivatechannel) or t.me link (e.g., t.me/myprivatechannel, https://t.me/myprivatechannel)
        if chat_identifier.startswith(("@", "t.me/", "https://t.me/")):
            # Strip prefixes and get the username
        ...
        # Handle invite link code (e.g., +code, t.me/+code, https://t.me/+code)
        if chat_identifier.startswith(("+", "t.me/+", "https://t.me/+")):
            invite_code = re.sub(r"^(https?://)?t\.me/|\+", "", chat_identifier)
            if not invite_code:
                raise ValueError(f"Invalid chat identifier: {chat_identifier}")
            # Attempt to join the chat to get the chat ID
    ...
    try:
        if user_id in USER_MESSAGE_IDS:
            ...
            for message in messages:
                if message.message_id == message_id and message.text and message.text.startswith(f"USER_{user_id}\n"):
                    data = json.loads(message.text.split("\n", 1)[1])
                    data["message_id"] = message_id
                    return data
        # Fallback: Search recent messages in storage chat
        messages = await context.bot.get_chat_history(chat_id=STORAGE_CHAT_ID, limit=100)
        for message in messages:
...
async def get_total_users(context: ContextTypes.DEFAULT_TYPE):
    ...
    count = 0
    try:
        messages = await context.bot.get_chat_history(chat_id=STORAGE_CHAT_ID, limit=1000)
        for message in messages:
...
def main():
    """Run the bot."""
    application = Application.builder().token(BOT_TOKEN).http_client_max_connections(20).http_client_pool_timeout(10).build()
